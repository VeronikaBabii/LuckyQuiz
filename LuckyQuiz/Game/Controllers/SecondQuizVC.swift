...
class SecondQuizVC: UIViewController {
    
    var quizLogic = QuizLogic2()
    let vcc = SecondWinVC()
...
//            vcc.onUserAction(data: "\(total)")
    ...
    func displayWinScreen() {
        let secondWinVC = storyboard?.instantiateViewController(identifier: Constants.Storyboard.secondWinVC) as? SecondWinVC
        
        view.window?.rootViewController = secondWinVC
        view.window?.makeKeyAndVisible()
    }
}
